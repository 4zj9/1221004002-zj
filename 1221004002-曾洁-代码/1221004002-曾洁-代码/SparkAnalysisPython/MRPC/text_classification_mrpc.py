...
# 解析命令行参数
parser = argparse.ArgumentParser(description='Spark文本分类实现 - MRPC数据集')
parser.add_argument('--data_file', type=str, default='hdfs://hadoop1:9000/user/spark/glue/MRPC/msr_paraphrase_train.txt', help='训练数据文件路径')
parser.add_argument('--test_file', type=str, default='hdfs://hadoop1:9000/user/spark/glue/MRPC/msr_paraphrase_test.txt', help='测试数据文件路径')
...
parser.add_argument('--output_fig', type=str, default='mrpc_model_comparison.png', help='输出图表文件名')
...
spark = SparkSession.builder \
    .appName("MRPC Text Classification") \
    .master("local[*]") \
...
def load_mrpc_data(file_path):
    ...
    try:
        ...
        data = data.withColumnRenamed("Quality", "label") \
            .withColumnRenamed("#1 String", "sentence1") \
            .withColumnRenamed("#2 String", "sentence2")

        # 将标签转换为数值
        data = data.withColumn("label", col("label").cast("double"))

        # 合并两个句子为一个文本字段
        data = data.withColumn("text", concat_ws(" [SEP] ", col("sentence1"), col("sentence2")))
        ...
        # 创建一个简单的示例数据集
        data = [("This is a sample sentence1 [SEP] This is sample sentence2", 0.0),
                ("Another example text1 [SEP] Another example text2", 1.0),
                ("A third text sample1 [SEP] A third text sample2", 0.0)]
...
def main():
    # 加载数据
    train_data = load_mrpc_data(args.data_file)
    test_data = load_mrpc_data(args.test_file)
